...
import { createValidator, type Validator, ValidatorBuilder } from "../../src/validation";
...
function example2_customRules() {
  ...
  const validator = new ValidatorBuilder<GameCard>()
    ...
    .custom("name", (name) => /^[A-Z]/.test(name), "Name must start with a capital letter")
    .custom("cost", (cost) => cost % 1 === 0, "Cost must be a whole number")
    .custom("text", (text) => text.length <= 200, "Card text must be at most 200 characters")
    .build();
