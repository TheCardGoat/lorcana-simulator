...
import type { CardId, CardZoneConfig, GameDefinition, MoveContext, PlayerId } from "@tcg/core";
 ...
 * Gundam Game Phase
 */
export type GundamPhase = "setup" | "start" | "draw" | "resource" | "main" | "end" | "gameOver";
 ...
 */
export const gundamGame: GameDefinition<GundamState, GundamMoves, GundamCard, GundamCardMeta> = {
  ...
  setup: (players) => {
    ...
    return {
      ...
      activeResources: Object.fromEntries(playerIds.map((id) => [id, 0])) as Record<
        PlayerId,
        number
      >,
      hasPlayedResourceThisTurn: Object.fromEntries(playerIds.map((id) => [id, false])) as Record<
        PlayerId,
        boolean
      >,
    };
  ...
  moves: {
    draw: {
      reducer: (draft, context) => {
        ...
        for (let i = 0; i < count; i++) {
          const deckCards = context.zones?.getCardsInZone("deck" as any, playerId);
    ...
    deployUnit: {
      condition: (state, context) => {
        ...
        const handCards = context.zones?.getCardsInZone("hand" as any, playerId);
        ...
        const battleArea = context.zones?.getCardsInZone("battleArea" as any, playerId);
    ...
    deployBase: {
      condition: (state, context) => {
        ...
        const baseSection = context.zones?.getCardsInZone("baseSection" as any, playerId);
    ...
    playResource: {
      condition: (state, context) => {
        ...
        const resourceArea = context.zones?.getCardsInZone("resourceArea" as any, playerId);
    ...
    attack: {
      ...
      reducer: (draft, context) => {
        ...
        if (targetId) {
          ...
          // Deal damage
          const newDamage = (targetMeta.damage ?? 0) + (attackerDef.attack ?? 0);
