...
function example1_basicZoneOperations() {
  ...
  console.log("card-999 in deck?", isCardInZone(deck, createCardId("card-999"))); // false
...
function example3_drawingCards() {
  ...
  const cards = Array.from({ length: 10 }, (_, i) => createCardId(`card-${i + 1}`));
...
function example4_shuffling() {
  ...
  console.log("Shuffled (seed 1 again):", getCardsInZone(shuffled2).slice(0, 5));
  console.log(
    "Same shuffle?",
    JSON.stringify(getCardsInZone(shuffled1)) === JSON.stringify(getCardsInZone(shuffled2)),
  );
...
function example5_searchingAndPeeking() {
  ...
  console.log("Deck still has:", getZoneSize(deck), "cards (peek doesn't modify)");
}
...
function example8_playerZones() {
  ...
  const playerZones = createPlayerZones<PlayerZones>([player1, player2], () => ({
    hand: createZone(
      {
        id: createZoneId("hand"),
        name: "hand",
...
function example11_completeGameScenario() {
  ...
  // Setup
  const cards = Array.from({ length: 30 }, (_, i) => createCardId(`card-${i + 1}`));
