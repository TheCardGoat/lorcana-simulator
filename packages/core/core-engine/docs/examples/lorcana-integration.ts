...
export const lorcanaFlow: FlowDefinition<LorcanaState> = {
  turn: {
    onBegin: (context) => {
      ...
      // Replenish ink
      context.state.ink.available[currentPlayer] = context.state.ink.total[currentPlayer];
 ...
 */
export const lorcanaGame: GameDefinition<LorcanaState, LorcanaMoves, LorcanaCard, LorcanaCardMeta> =
  {
    name: "Disney Lorcana",
    ...
    setup: (players) => {
      ...
      return {
        ...
        lore: Object.fromEntries(playerIds.map((id) => [id, 0])) as Record<PlayerId, number>,
        ink: {
          available: Object.fromEntries(playerIds.map((id) => [id, 0])) as Record<PlayerId, number>,
          total: Object.fromEntries(playerIds.map((id) => [id, 0])) as Record<PlayerId, number>,
        },
        turnMetadata: {
          cardsPlayedThisTurn: [],
          charactersQuesting: [],
    ...
    moves: {
      playCard: {
        condition: (state, context) => {
          ...
          const handCards = context.zones?.getCardsInZone("hand" as any, currentPlayer);
...
export function exampleUsage() {
  ...
  if (result.success) {
    ...
    console.error("Move failed:", "error" in result ? result.error : "Unknown error");
  }
