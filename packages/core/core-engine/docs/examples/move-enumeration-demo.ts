...
const attackMove: MoveDefinition<DemoGameState, AttackParams> = {
  ...
  enumerator: (state, context) => {
    ...
    // Must be current player
    if (state.players[state.currentPlayerIndex]?.id !== context.playerId) return [];
  ...
  condition: (state, context) => {
    ...
    const targetIsPlayer = state.players.some((p) => p.id === context.params.targetId);
    const targetIsCreature = state.players.some((p) => p.field.includes(context.params.targetId));
  ...
  reducer: (draft, context) => {
    console.log(`${context.params.attackerId} attacks ${context.params.targetId}!`);

    // Check if target is a player or creature
    const targetPlayer = draft.players.find((p) => p.id === context.params.targetId);
...
const passPhaseMove: MoveDefinition<DemoGameState, PassPhaseParams> = {
  ...
  reducer: (draft) => {
    // Advance phase
    const phaseOrder: Array<DemoGameState["phase"]> = ["draw", "main", "battle", "end"];
...
const demoGameDefinition: GameDefinition<DemoGameState, DemoMoves> = {
  ...
  setup: (players) => ({
    players: players.map((p) => ({
      ...
      field: [],
      deck: ["card5", "card6", "card7", "card8", "card9", "card10", "card11", "card12"],
      life: 20,
      mana: 3,
      maxMana: 3,
    })),
  ...
  endIf: (state) => {
    ...
    const noCards = state.players.find((p) => p.deck.length === 0 && p.hand.length === 0);
    if (noCards) {
 ...
 */
function simpleAI(engine: RuleEngine<DemoGameState, DemoMoves>, playerId: string) {
  ...
  if (bestMove) {
    console.log(`✅ AI chooses: ${bestMove.metadata?.displayName || bestMove.moveId}`);
 ...
 */
function displayAvailableMoves(engine: RuleEngine<DemoGameState, DemoMoves>, playerId: string) {
