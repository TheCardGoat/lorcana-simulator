...
const simpleGameDefinition: GameDefinition<SimpleGameState, SimpleGameMoves> = {
    ...
    players: players.map((p) => ({
      ...
      deck: shuffle(
        createZone(
          ...
          Array.from({ length: 30 }, (_, i) => createCardId(`${p.id}-card-${i + 1}`)),
        ),
        `${p.id}-shuffle`,
      ),
  ...
  moves: {
    ...
    playCard: {
      condition: (state, context) => {
        ...
        return player !== undefined && state.phase === "main" && player.hand.cards.includes(cardId);
      },
      reducer: (draft, context) => {
        const player = draft.players.find((p) => p.id === context.playerId);
        if (!player) return;
        ...
        const cardId = context.data?.cardId as CardId;
        const { fromZone, toZone } = moveCard(player.hand, player.graveyard, cardId);
    ...
    dealDamage: {
      reducer: (draft, context) => {
        ...
        const opponentIndex = (draft.currentPlayerIndex + 1) % draft.players.length;
  ...
  endIf: (state) => {
    ...
    if (loser) {
      ...
      return winner ? { winner: winner.id, reason: "Opponent life reached 0" } : undefined;
    }
    return undefined;
  },
};
...
describe("Example 5: Test Utilities - Factories", () => {
  ...
  test("should create test zones", () => {
    const deck = createTestDeck([createCardId("card-1"), createCardId("card-2")], "p1");
...
describe("Example 6: Deterministic RNG Testing", () => {
  ...
  test("should use withSeed for deterministic operations", () => {
    ...
    const shuffled3 = withSeed("different-seed", (rng) => rng.shuffle([...array]));
