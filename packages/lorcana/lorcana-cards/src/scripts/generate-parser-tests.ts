...
function flattenCardText(text: Card["text"]): string {
  if (!text) return "";
...
function generateTestFile(setNumber: string, cards: Card[]): string {
  const cardsWithText = cards.filter((card) => flattenCardText(card.text).trim());

  const sortedCards = cardsWithText.toSorted((a, b) => a.cardNumber - b.cardNumber);

  const testCases = sortedCards.map((card) => {
    const displayName = getCardDisplayName(card);
    const escapedText = escapeString(flattenCardText(card.text));
