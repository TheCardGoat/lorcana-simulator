...
import type { ConditionalEffect, Effect, OptionalEffect, SequenceEffect } from "../../types";
...
function parseFromText(text: string): OptionalEffect | SequenceEffect | null {
  ...
  if (ifYouDoMatch) {
    ...
    const conditionalStep: ConditionalEffect = {
    ...
    return {
      steps: [optionalStep, conditionalStep],
      type: "sequence",
    };
  }
