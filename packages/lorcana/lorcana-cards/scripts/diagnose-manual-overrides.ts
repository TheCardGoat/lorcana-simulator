...
const CANONICAL_CARDS_PATH = path.resolve(__dirname, "../src/data/canonical-cards.json");
...
function loadCanonicalCards(): Record<string, CanonicalCard> {
  if (!fs.existsSync(CANONICAL_CARDS_PATH)) {
    console.error(`❌ Error: ${CANONICAL_CARDS_PATH} not found. Run generate-cards.ts first.`);
    process.exit(1);
  }
...
function main(): void {
  ...
  console.log(`  📝 Total manual override entries: ${Object.keys(MANUAL_ENTRIES).length}`);
  ...
  if (matches.length > 0) {
    ...
    matches.slice(0, 5).forEach((result, i) => {
      ...
      console.log(`     Original: "${result.originalText.substring(0, 80)}..."`);
  ...
  if (nonMatches.length > 0) {
    ...
    nonMatches.slice(0, 10).forEach((result, i) => {
      ...
      if (existsInManual) {
        console.log(`     ⚠️  Pattern EXISTS in MANUAL_ENTRIES but wasn't matched!`);
      } else {
        // Find closest match
  ...
  if (matches.length < Object.keys(MANUAL_ENTRIES).length) {
    ...
    console.log("   This suggests the normalization fix should help increase matches.");
  }
}
