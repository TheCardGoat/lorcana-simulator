...
describe("validateDeckForFormat", () => {
  ...
  describe("format definitions", () => {
    ...
    it("infinity does not include WUN in allowedSets", () => {
      expect(LORCANA_FORMATS.infinity.allowedSets).not.toContain("WUN");
    });

    it("core-constructed excludes WUN even when rotation matches", () => {
      expect(LORCANA_FORMATS["core-constructed"].excludedSets).toContain("WUN");
    });

    it("early-access still allows WUN", () => {
      expect(LORCANA_FORMATS["early-access"].allowedSets).toContain("WUN");
      expect(LORCANA_FORMATS["early-access"].excludedSets ?? []).not.toContain("WUN");
    });

    it("a WUN-only card is illegal in infinity and core-constructed but legal in early-access", () => {
      const lookup = buildLookup({
        wun: card("wun", {
          sets: ["WUN"] as LorcanaSetCode[],
          rotationStates: ["CoreConstructed"],
      ...
      const ea = validateDeckForFormat(deckCards, lookup, LORCANA_FORMATS["early-access"]);
      expect(ea.rules.find((r) => r.kind === "CARD_SET")?.passed).toBe(true);
    });
  });
});
