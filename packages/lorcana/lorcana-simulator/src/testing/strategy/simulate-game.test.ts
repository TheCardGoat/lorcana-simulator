...
const RUN_STRATEGY_HEAD_TO_HEAD_BENCHMARK =
  process.env.RUN_STRATEGY_HEAD_TO_HEAD_BENCHMARK === "1";
...
type StrategyTraceSummary = Pick<
  AutomatedActionDecisionTrace,
  "fallbackTaken" | "strategyName"
>;

function sanitizeDeckId(value: string): string {
...
function createBenchmarkDecks(fixtures: readonly DeckFixture[]): StrategyBenchmarkDeck[] {
  return fixtures.map((fixture) => ({
    fixture,
    id: sanitizeDeckId(fixture.name),
  }));
}
...
function summarizeBenchmarkBatch(matches: readonly StrategyBenchmarkMatchSummary[]) {
  const totals = matches.reduce(
    ...
    },
    {
      ...
      games: 0,
      orderedPairTotals: {} as Record<
        string,
        { games: number; totalMargin: number; wins: number }
      >,
      totalActions: 0,
      totalLoreMargin: 0,
      totalTurns: 0,
...
function runMirroredHeadToHeadBenchmark(
  ...
  permutations.forEach((permutation, index) => {
    ...
    for (const game of games) {
      ...
      for (const trace of readStrategyTraceSummaries(game.summary.artifactPaths.strategyDecisions)) {
...
describe("strategy deck simulation convenience method", () => {
  it(
    ...
    () => {
      ...
      expect(summary.gameEndReason).toBe("Reached 20 lore");
      expect(summary.fallbackCounts.concede).toBe(0);
  ...
  it.if(RUN_STRATEGY_BATCH_BENCHMARK)(
    ...
    () => {
      ...
      expect(
        legacyBatch.matches.every((summary) => summary.gameEndReason === "Reached 20 lore"),
      ).toBe(true);
      expect(
        defaultBatch.matches.every((summary) => summary.gameEndReason === "Reached 20 lore"),
      ).toBe(true);
      expect(
        legacyBatch.matches.every((summary) => summary.fallbackCounts.concede === 0),
      ).toBe(true);
      expect(
        defaultBatch.matches.every((summary) => summary.fallbackCounts.concede === 0),
      ).toBe(true);
    },
    { timeout: 600000 },
  );
  ...
  it.if(RUN_STRATEGY_HEAD_TO_HEAD_BENCHMARK)(
    ...
    () => {
      ...
      expect(summary.strategyConcedeFallbacks[defaultLoreRaceAutomatedActionStrategy.name]).toBe(
        0,
      );
    },
    { timeout: 600000 },
