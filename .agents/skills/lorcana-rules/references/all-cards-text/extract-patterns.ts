...
function categorizePattern(pattern: string): PatternCategory {
  ...
  if (/^(rush|ward|alert|vanish|evasive|support|reckless|bodyguard)$/i.test(pattern)) {
    return "keyword";
  }
  ...
  // Damage patterns
  if ((lower.includes("deal") && lower.includes("damage")) || lower.includes("damage counter")) {
    return "damage";
  }
  ...
  // Quest restrictions
  if (lower.includes("quest") && (lower.includes("can't") || lower.includes("must"))) {
  ...
  // Inkwell
  if (lower.includes("inkwell") || (lower.includes("pay") && lower.includes("less"))) {
    return "inkwell";
  }
  ...
  // Location
  if (lower.includes("location") || lower.includes("move") || lower.includes("while here")) {
    return "location";
  }
  ...
  // Replacement effects
  if (lower.includes("instead") || (lower.includes("would") && lower.includes("don't"))) {
 ...
 */
export function extractPatterns(texts: string[] = allCardsText): PatternAnalysis {
...
export function formatPatternReport(analysis: PatternAnalysis): string {
  ...
  lines.push(`- **Pattern Categories:** ${Object.keys(analysis.categoryCounts).length}`);
  lines.push("");
  ...
  for (const [category, count] of sortedCategories) {
    ...
    const categoryPatterns = analysis.patterns.filter((p) => p.category === category).slice(0, 10); // Show top 10 per category
...
export function exportPatternsTypeScript(analysis: PatternAnalysis): string {
  ...
  for (const [category, count] of Object.entries(analysis.categoryCounts)) {
    ...
    const categoryPatterns = analysis.patterns.filter((p) => p.category === category);
    ...
    for (const pattern of categoryPatterns) {
      ...
      lines.push(`    examples: ${JSON.stringify(pattern.examples.slice(0, 2))},`);
